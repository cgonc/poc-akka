package com.akka.demo.a008.blockchain;

import com.akka.demo.a008.model.Block;
import com.akka.demo.a008.model.HashResult;
import com.akka.demo.a008.utils.BlockChainUtils;
	...
	public Receive<Command> createReceive() {
		return newReceiveBuilder().onAnyMessage(message -> {
			...
			while(!hash.substring(0, message.getDifficulty()).equals(target) && nonce < message.getStartNonce() + 1000){
				nonce++;
				String dataToEncode =
						message.getBlock().getPreviousHash() + Long.toString(message.getBlock().getTransaction().getTimestamp()) + Integer.toString(nonce) + message.getBlock().getTransaction();
