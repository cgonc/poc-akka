...
public class Racer extends AbstractBehavior<Racer.Command> {
	...
	public Receive<Command> waitingToStop() {
		return newReceiveBuilder().onAnyMessage(message -> Behaviors.same()).onSignal(PostStop.class, signal -> {
			log.info("I {} am about the terminate...", getContext().getSelf().path().toString());
			return Behaviors.same();
		}).build();
	}
